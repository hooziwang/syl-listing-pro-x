...
必须显式传入 --base-url。默认会检查 /healthz、/v1/auth/exchange、/v1/rules/resolve、/v1/rules/refresh 和规则下载链路；如果传 --with-generate，还会额外发起一次真实生成并订阅 SSE 任务事件。
这个命令不会改远端代码，但会访问真实 worker 服务和真实租户规则。`,
		Example: `  syl-listing-pro-x worker diagnose-external --base-url https://worker.example.test --key <SYL_LISTING_KEY>
  syl-listing-pro-x worker diagnose-external --key <SYL_LISTING_KEY> --with-generate`,
	...
	cmd.Flags().StringVar(&baseURL, "base-url", "", "worker 对外地址")
